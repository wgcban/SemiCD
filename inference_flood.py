...
def multi_scale_predict(model, image, scales, num_classes, flip=True):
    ...
    for scale in scales:
        ...
        scaled_prediction = upsample(model(scaled_img)[0])

        if flip:
            fliped_img = scaled_img.flip(-1)
            fliped_predictions = upsample(model(fliped_img)[0])
...
def main():
    ...
    for index, data in enumerate(tbar):
        ...
        with torch.no_grad():
            ...
            output_classification_prob = model(image)[1].cpu().numpy()
            output_classification = np.argmax(output_classification_prob)

        output_resized  = cv2.resize(output.transpose(1,2,0), (4000,3000), interpolation=cv2.INTER_LINEAR).transpose(2,0,1)
        prediction      = np.asarray(np.argmax(output_resized, axis=0), dtype=np.uint8)

        # Write classifications to dict
        dict = {image_id[0]: output_classification.item()}
        prediction_dict.update(dict)

        # SAVE RESULTS
        cv2.imwrite('outputs/submission/'+image_id[0]+'.png', prediction.astype(np.uint8))

        prediction_im = colorize_mask(prediction, palette)
        prediction_im.save('outputs/'+image_id[0]+'.png')
    
    with open("./outputs/submission/image_classes.json", "w+") as write_file:
        json.dump(prediction_dict, write_file)
